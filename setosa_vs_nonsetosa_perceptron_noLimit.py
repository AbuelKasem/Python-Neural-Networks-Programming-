...
def setosa_model_train(training_data,desired_output,iter_limit,learning_rate):
  n_err=0
  n_correct=0
  error=1  #init. error to zero ,this error is between desired output and estimated output based on decision boundary evaluation
...
setosa_model_weights,setosa_model__out=setosa_model_train(training_set,setosa_desired_classification,n_it,l) #call the training function ,pass training data set,desired output ,number of iterations ,learning rate
...
print(f'setosa_model__out={setosa_model__out}') 
'''
...
d2=setosa_classify(test_s,w_f) #pass data to classifier with the trained model 
print(f'd2={d2}')  
